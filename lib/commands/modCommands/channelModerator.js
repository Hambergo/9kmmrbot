...
const addChannelModerator = async function addChannelModerator(db, channel, args) {
	if (!args[0]) {
		return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot addmod <username>'))
	}
...
/* Removes a user from the 9kmmrbot mods in the channel */
const deleteChannelModerator = async function deleteChannelModerator(db, channel, args) {
	if (!args[0]) {
		return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot delmod <username>'))
	}

	const userToMod = args[0].toLowerCase()
	const { id } = await GetUserInfoByLogin(userToMod) || {}
	...
	if (!channel.mods || !channel.mods.includes(id)) {
		return Promise.reject(new CustomError(`${userToMod} is not a mod in this channel.`))
	}

	await db.collection('channels')
		.updateOne(
			{ id: channel.id },
			{ $pull: { mods: id }, $setOnInsert: { count: 0 } },
			{ upsert: true }
		)

	return `Successfully removed ${userToMod} from ${channel.name} 9kmmrbot mods`
}

module.exports = { addChannelModerator, deleteChannelModerator }
