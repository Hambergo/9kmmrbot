	...
	function: async (channel, userstate, message) => {
		...
		if (split.length == 2 && split[1].toLowerCase() == 'listacc') {
			...
			if (split.length == 3) {
				...
				if (isNaN(id)) {
					return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot addacc id'))
				} else {
					let temp = []
					temp.push({ accounts: { $exists: false } })
					for (let i = 0; i < 10; i++) {
						temp.push({ accounts: { $size: i } })
					}
					let result = await mongoDb().collection('channels').updateOne({ id: userstate['room-id'], $or: temp }, { $addToSet: { accounts: id } })
					if (result.matchedCount) {
						return `${userstate.username} successfully added ${id} to ${channel.substring(1)} accounts`
					} else {
			...
			if (split.length == 3) {
				let id = util.returnAccountId(split[2])
				if (isNaN(id)) {
					return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot delacc id'))
				} else {
					let result = await mongoDb().collection('channels').updateOne({ id: userstate['room-id'] }, { $pull: { accounts: id } })
					if (result.matchedCount) {
						return `${userstate.username} successfully removed ${id} from ${channel.substring(1)} accounts`
					} else {
						return Promise.reject(new CustomError(`${id} isn't listed under ${channel.substring(1)} accounts`))
					}
				}
			} else {
				return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot delacc id'))
