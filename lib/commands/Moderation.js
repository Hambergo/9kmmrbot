...
let command = {
	...
	function: async (channel, userstate, message) => {
		...
		if (split.length == 2 && split[1].toLowerCase() == 'listacc') {
			...
			return addAccount(userstate.username, userstate['room-id'], channel.substring(1), id)
			...
			return deleteAccount(userstate.username, userstate['room-id'], channel.substring(1), id)
...
async function addAccount(username, channelId, channelName, steamId) {
	const chan = await mongoDb().collection('channels').findOne({ id: channelId })

	/* Check to see if there are already 10 accounts linked to this channel,
	 * or if the account has already been linked. */
	if (!chan) {
		return Promise.reject(new CustomError(`Cannot find ${channelName} in the collection`))
	}
			
	if (chan.accounts.length > 10) {
		return Promise.reject(new CustomError('Can\'t add more than 10 accounts for a stream'))
	}

	if (chan.accounts.some(account => account == steamId)) {
		return Promise.reject(new CustomError(`Account ${steamId} is already connected to ${channelName}`))
	}

	// All good, link the account to the channel.
	await mongoDb().collection('channels').updateOne({ id: channelId }, { $addToSet: { accounts: steamId } })
	return `${username} successfully added ${steamId} to ${channelName} accounts`
}

/* Removes a Steam ID from the channel's list of linked accounts */
async function deleteAccount(username, channelId, channelName, steamId) {
	const chan = await mongoDb().collection('channels').findOne({ id: channelId })
	...
	if (!chan.accounts.some(account => account == steamId)) {
		return Promise.reject(new CustomError(`Account ${steamId} is not linked to ${channelName}`))
	}

	await mongoDb().collection('channels').updateOne({ id: channelId }, { $pull: { accounts: steamId } })
	return `${username} successfully removed ${steamId} from ${channelName} accounts`
}
