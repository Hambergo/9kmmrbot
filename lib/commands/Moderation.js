...
const { canInvokeModCommands, canInvokeChannelAdminCommands } = require('../helpers/modHelper')
...
async function handleCommand(user, channel, args) {
	...
	switch (command) {
		...
		case 'addnp':
			return addChannelNotablePlayer(db, user, channel, args)
		case 'delnp':
			return deleteChannelNotablePlayer(db, user, channel, args)
		case 'delay':
		...
		default:
			return handleChannelAdminCommands(command, user, channel, args)
	}
}

/* Handles channel-level admin commands that require Global Mod access (or to be the streamer)*/
async function handleChannelAdminCommands(command, user, channel, args) {
	if (!canInvokeChannelAdminCommands(user.id, user.isGlobalMod, channel.id)) {
		return Promise.reject()
	}
	...
	switch (command) {
		...
		case 'delmod':
			return deleteChannelModerator(db, channel, args)
		default:
			return handleGlobalModCommand(command, user, channel, args)
	}
}

/* Handles admin commands that require Global Mod access */
async function handleGlobalModCommand(command, user, channel, args) {
	if (!user.isGlobalMod) {
		return Promise.reject()
	}
	...
	switch (command) {
		case 'addglobalnp':
			return addGlobalNotablePlayer(db, user, channel, args)
		case 'delglobalnp':
			return deleteGlobalNotablePlayer(db, user, channel, args)
		case 'join':
			return joinChannel(db, args)
		case 'part':
			return partChannel(db, args)
		default:
			return Promise.reject()
	}
}
