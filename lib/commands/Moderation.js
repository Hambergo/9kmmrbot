...
const { listAccounts, addAccount, deleteAccount } = require('./modCommands/linkedAccounts')
const { addNotablePlayer, deleteNotablePlayer } = require('./modCommands/notablePlayer')
...
let command = {
	...
	function: async (roomName, userstate, message) => {
		...
		const isGlobalMod = await db.collection('settings').countDocuments({ name: 'Global Mod', id: channelId })
		...
		if (split.length == 2 && split[1].toLowerCase() == 'delay') {
			let delayedChannel = db.collection('settings').findOne({ id: channelId, name: 'Delayed Channel', delayed: true }, { projection: { _id: 0 } })
			...
			if (split[2].toLowerCase() == 'on') {
				await db.collection('settings').updateOne({ id: channelId, name: 'Delayed Channel' }, { $set: { delayed: true } }, { upsert: true })
				return 'Turned delay on'
			} else if (split[2].toLowerCase() == 'off') {
				await db.collection('settings').updateOne({ id: channelId, name: 'Delayed Channel' }, { $set: { delayed: false } }, { upsert: true })
				...
				if (!isNaN(tempNumber) && tempNumber > 0 && tempNumber <= 600 && tempNumber % 30 == 0) {
					await db.collection('settings').updateOne({ id: channelId, name: 'Delayed Channel' }, { $set: { delay: tempNumber } }, { upsert: true })
			...
			if (split[2].toLowerCase() == 'set' && split.length == 4) {
				...
				if (!isNaN(tempNumber) && tempNumber >= 30 && tempNumber <= 300) {
					if (tempNumber == 30) {
						db.collection('channels').updateOne({ id: channelId }, { $unset: { cooldown: '' } })
					} else {
						db.collection('channels').updateOne({ id: channelId }, { $set: { cooldown: tempNumber } })
					}
			...
			let includeSelf = db.collection('settings').findOne({ id: channelId, name: 'Include Self Channel' }, { projection: { _id: 0 } })
			if (await includeSelf) {
				db.collection('settings').deleteOne({ id: channelId, name: 'Include Self Channel' })
				return 'Successfully toggled showing streamer as a notable player off'
			} else {
				db.collection('settings').insertOne({ id: channelId, name: 'Include Self Channel' })
				return 'Successfully toggled showing streamer as a notable player on'
			}
		} else if (split[1].toLowerCase() == 'toggleemotes') {
			let emote = db.collection('settings').findOne({ id: channelId, name: 'emotes Channel' })
			emote = await emote
			if (emote) {
				db.collection('settings').deleteOne({ id: channelId, name: 'emotes Channel' })
				return 'Successfully toggled hero emotes off'
			} else {
				db.collection('settings').insertOne({ id: channelId, name: 'emotes Channel' })
			...
			let heroes = db.collection('heroes').find({ id: { $in: game.players.map(player => player.hero_id.toString()) } }).toArray()
			heroes = await heroes
					...
					return util.fetch('https://api.twitch.tv/kraken/users?login=' + split[2], { headers: util.twitchAPIHeaders }).then(res => res.json()).then(async json => {
						if (json.users && json.users.length && json.users.some(value => value.name.toLowerCase() == split[2].toLowerCase())) {
							if (mods.find(mod => mod == json.users.find(value => value.name.toLowerCase() == split[2].toLowerCase())._id)) {
								...
								await db.collection('channels').updateOne({ id: channelId }, { $addToSet: { mods: json.users.find(value => value.name.toLowerCase() == split[2].toLowerCase())._id }, $setOnInsert: { count: 0 } }, { upsert: true })
