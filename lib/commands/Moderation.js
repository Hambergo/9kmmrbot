...
let command = {
	...
	function: async (roomName, userstate, message) => {
		...
		return handleCommand(username, channel, args)

		if (split[1].toLowerCase() == 'addnp') {
			if (split.length > 3) {
				...
				if (isNaN(id)) {
					...
					id = id.toString()
					await mongoDb().collection('notable players').updateOne({ id, channel: channelId }, { $set: { id, channel: channelId, name: split.slice(3).join(' '), lastChanged: new Date(), lastChangedBy: userId, enabled: true } }, { upsert: true })
					return `${username} successfully added ${id} to ${channelName} local notable players`
				}
			} else {
				return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot addnp id nickname'))
			}
			...
			if (split.length == 3) {
				...
				if (isNaN(id)) {
					...
					id = id.toString()
					await mongoDb().collection('notable players').updateOne({ id, channel: channelId }, { $set: { id, channel: channelId, lastChanged: new Date(), lastChangedBy: userId, enabled: false } })
					return `${username} successfully removed ${id} from ${channelName} local notable players`
				}
			} else {
				return Promise.reject(new CustomError('Wrong syntax: !9kmmrbot delnp id'))
			}
...
async function handleCommand(username, channel, args) {
	...
	switch (command) {
		...
		case 'addacc':
			return addAccount(username, channel, args)
		case 'delacc':
			return deleteAccount(username, channel, args)
		default:
			return Promise.reject()
	}
}
...
async function addAccount(username, channel, args) {
...
async function deleteAccount(username, channel, args) {
