...
function canInvokeModCommands(userId, isGlobalMod, channelId, channelMods) {
	if (canInvokeChannelAdminCommands(userId, isGlobalMod, channelId))
		return true

	return channelMods && (channelMods.indexOf(userId) >= 0)
}

/* Checks whether the user has privileges to invoke channel admin commands */
function canInvokeChannelAdminCommands(userId, isGlobalMod, channelId) {
