...
function canInvokeModCommands(userId, isGlobalMod, channelId, channelMods) {
	if (canInvokeGlobalModCommands(userId, isGlobalMod, channelId))
		return true

	return channelMods && (channelMods.indexOf(userId) >= 0)
}

/* Checks whether the user has privileges to invoke global moderation commands in the channel */
function canInvokeGlobalModCommands(userId, isGlobalMod, channelId) {
	return ((userId == channelId) || isGlobalMod)
}
