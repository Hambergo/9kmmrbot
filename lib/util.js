...
const findGameFromChannel = async (roomid, allowSpectating = true, allowCustomGame = true) => {
	...
	let games = mongoDb().collection('games').aggregate([{ $match: { createdAt: { $gte: new Date(new Date() - 900000) } } }, { $group: { _id: '$createdAt', matches: { $addToSet: '$$ROOT' } } }, { $sort: { _id: -1 } }, { $skip: index }, { $limit: 1 }]).toArray()
	games = await games
		...
		if (game) {
			if (game.lobby_type == 1) {
				...
				if (dac && dac.players && dac.players.some(player => accounts.find(account => player.account_id == account))) {
					...
					let tempGame = await SteamApi.GetRealTimeStats({ server_steam_id: game.server_steam_id })
					if (game.players) {
			...
			let rps = mongoDb().collection('rps').aggregate([{ $match: { createdAt: { $gte: new Date(new Date() - 900000) } } }, { $group: { _id: '$createdAt', rps: { $addToSet: '$$ROOT' } } }, { $sort: { _id: -1 } }, { $skip: index }, { $limit: 1 }]).toArray()
			rps = await rps
			if (rps && rps.length) {
				rps = rps[0].rps
				let rp = rps.find(rp => accounts.some(account => rp.account_id == account) && ((rp.status == '#DOTA_RP_SPECTATING' || rp.status == '#DOTA_RP_FINDING_MATCH' || rp.status == '#DOTA_RP_WATCHING_TOURNAMENT') && rp.watching_server || rp.WatchableGameID))
				if (rp) {
					...
					if (!game || game.lobby_type == 1) {
						let server_steam_id = (game && game.server_steam_id) || rp.watching_server
						if (server_steam_id) {
							let dac = await mongoDb().collection('dac').findOne({ server_steam_id: server_steam_id.toString() })
							let tempGame
							if (!dac) {
								tempGame = await SteamApi.GetRealTimeStats({ server_steam_id: server_steam_id })
								if (!tempGame.players.some(player => player.hero_id == 0)) {
	...
	if (rps && rps.length) {
		...
		if (account) {
			...
			let game = await SteamApi.GetRealTimeStats({ server_steam_id: server_steam_id })
