		...
		if (game) {
			...
			if (rps && rps.length) {
				...
				if (rp) {
					...
					rp = rps.find(rp => accounts.some(account => rp.account_id == account) && (rp.status == '#DOTA_RP_PLAYING_AS' && rp.param0 == '#DOTA_lobby_type_name_bot_match' || rp.status == '#DOTA_RP_BOTPRACTICE'))
					if (rp) {
						return Promise.reject(new CustomError('MrDestructoid Currently losing to OpenAI MrDestructoid'))
					}
				}
			}
		}
